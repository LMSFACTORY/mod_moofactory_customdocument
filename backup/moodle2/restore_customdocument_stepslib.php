...
/**
 * Define all the restore steps that will be used by the restore_customdocument_activity_task
 */

/**
 * Structure step to restore one customdocument activity
 */
class restore_customdocument_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('customdocument', '/activity/customdocument');

        if ($userinfo) {
            $paths[] = new restore_path_element('customdocument_issue', '/activity/customdocument/issues/issue');
        }
    ...
    protected function process_customdocument($olddata) {
        global $DB;
        ...
        // Insert the customdocument record.
        $newitemid = $DB->insert_record('customdocument', $data);
        // Immediately after inserting "activity" record, call this.
        $this->apply_activity_instance($newitemid);
    }

    protected function process_customdocument_issue($data) {
        global $DB;
        ...
        $data->certificateid = $this->get_new_parentid('customdocument');
        $data->userid = $this->get_mappingid('user', $data->userid);
        if (!isset($data->timecreated)) {
            $data->timecreated = time();
        }

        $newitemid = $DB->insert_record('customdocument_issues', $data);
        $this->set_mapping('customdocument_issue', $oldid, $newitemid);
