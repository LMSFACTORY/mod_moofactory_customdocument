...
$strcertificates = get_string('modulenameplural', 'customdocument');
$strissued  = get_string('issued', 'customdocument');
$strname  = get_string("name");
$strsectionname = get_string('sectionname', 'format_'.$course->format);

// Print the header.
$PAGE->set_pagelayout('incourse');
$PAGE->set_url('/mod/customdocument/index.php', array('id' => $course->id));
...
$certificates = get_all_instances_in_course('customdocument', $course);

if (!$certificates) {
    echo $OUTPUT->header();
    notice(get_string('nocertificatesissued', 'customdocument'), "$CFG->wwwroot/course/view.php?id=$course->id");
    echo $OUTPUT->footer();
    exit();
}
...
foreach ($certificates as $certificate) {
    if (!$certificate->visible) {
        ...
        $link = html_writer::tag('a', $certificate->name, array('class' => 'dimmed',
            'href' => $CFG->wwwroot . '/mod/customdocument/view.php?id=' . $certificate->coursemodule));
    } else {
        // Show normal if the mod is visible.
        $link = html_writer::tag('a', $certificate->name, array('href' => $CFG->wwwroot .
          '/mod/customdocument/view.php?id=' . $certificate->coursemodule));
    }
    ...
    if ($certrecord = $DB->get_record('customdocument_issues', array('userid' => $USER->id,
        'certificateid' => $certificate->id))) {
        $issued = userdate($certrecord->timecreated);
    } else {
        $issued = get_string('notreceived', 'customdocument');
    }
