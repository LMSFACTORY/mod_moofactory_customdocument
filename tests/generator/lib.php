...
/**
 * mod_customdocument data generator.
 *
...
/**
 * mod_customdocument data generator class.
 *
 * @package    mod_customdocument
 * @category   test
 * @copyright  2013 Carlos Alexandre S. da Fonceca
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_customdocument_generator extends testing_module_generator {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        $defaultsettings = array(
                ...
                'secondpagex'      => 0,
                'secondpagey'      => 50,
                'width'            => get_config('customdocument', 'width'),
                'height'           => get_config('customdocument', 'height'),
                'printqrcode'      => 1,
                'codex'            => 30,
                'codey'            => 130,
        ...
        if (!isset($record->certificatetext['text'])) {
            $record->certificatetext['text'] = file_get_contents(
                "$CFG->dirroot/mod/customdocument/tests/fixtures/firstpage.html"
            );
        ...
        if (!isset($record->secondpagetext['text'])) {
            $record->secondpagetext['text'] = file_get_contents(
                "$CFG->dirroot/mod/customdocument/tests/fixtures/secondpage.html"
            );
        ...
        if (!isset($record->certificateimage)) {
            $record->certificateimage = $CFG->dirroot . '/mod/customdocument/tests/fixtures/firstpagetestimage.jpg';
        }

        if (!isset($record->secondimage)) {
            $record->secondimage = $CFG->dirroot . '/mod/customdocument/tests/fixtures/secondpagetestimage.jpg';
        }
