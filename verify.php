...
$PAGE->set_url('/mod/customdocument/verify.php', array('code' => $code));
$PAGE->set_context($context);
$PAGE->set_title(get_string('certificateverification', 'customdocument'));
$PAGE->set_heading(get_string('certificateverification', 'customdocument'));
$PAGE->set_pagelayout('base');
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('certificateverification', 'customdocument'));
...
if (!$verifyform->get_data()) {
    ...
    $strto = get_string('awardedto', 'customdocument');
    $strdate = get_string('issueddate', 'customdocument');
    ...
    $table->data[] = array($coursename, $username,
            userdate($issuedcert->timecreated) . customdocument_print_issue_certificate_file($issuedcert), $issuedcert->code);
    echo html_writer::table($table);

    // Add to log.
    $event = \mod_customdocument\event\certificate_verified::create(array(
...
function get_issued_cert($code = null) {
    ...
    $issuedcert = $DB->get_record("customdocument_issues", array('code' => $code));
    if (!$issuedcert) {
        print_error('invalidcode', 'customdocument');
    }
    return $issuedcert;
}
...
function get_course_name($issuedcert) {
    ...
    return get_string('coursenotfound', 'customdocument');
}
