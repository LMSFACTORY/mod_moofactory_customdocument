...
class customdocument {
    ...
    protected function send_alert_email_teachers($issuedcert) {
        ...
        /******* Modif PRT 230424 *******/
        // Notification uniquement aux enseignants (managers) de la même organisation que le user
        $shortname = "Organisation";
        $userid = $issuedcert->userid;

        $sql = "SELECT {user_info_data}.data";
        $sql .= "  FROM {user_info_field} INNER JOIN {user_info_data}";
        $sql .= "    ON {user_info_data}.fieldid = {user_info_field}.id";
        $where = "  WHERE {user_info_field}.shortname ='" .$shortname. "' AND {user_info_data}.userid = ".$userid;
        $finalsql = $sql . $where;
        $result = $DB->get_records_sql($finalsql);
        ...
        if (!empty($this->get_instance()->emailteachers) && $teachers) {
            ...
            foreach ($teachers as $teacher) {
                ...
                $result = $DB->get_records_sql($finalsql);
                $tab = array_values($result);
                $teacherorg = $tab[0]->data;

                if($teacherorg == $userorg){
                    $emailteachers[] = $teacher->user->email;
                }
            }           
            /******* Fin Modif PRT 230424 *******/
        ...
        if (!empty($emails)) {
            ...
            foreach ($emails as $email) {
                ...
                if (validate_email($email)) {
                    ...
                    if ($file) {
                        @email_to_user($destination, $from, $postsubject, $posttext, $posthtml, $relativefilepath, $file->get_filename()); // If it fails, oh well, too bad.
                    }
                    else{
