...
/**
 * Basic customdocument steps definitions.
 *
...
class behat_mod_customdocument extends behat_base {
    // Just a memo note.
    ...
    public function add_grade_restriction($activity, $gradeactivity, $mingrade) {
        ...
        $this->execute('behat_forms::i_select_from_the_singleselect',
             array($this->escape($gradeactivity), get_string('printgrade', 'customdocument'))
        );
    ...
    public function i_am_on($pagename) {
        switch ($pagename) {
            case "certificate verification page":
                $this->getSession()->visit($this->locate_path('mod/customdocument/verify.php'));
            break;
    ...
    private function get_issue_certificate_by_customdocument_name($certificatename, $user) {
        global $DB, $CFG;
        require_once($CFG->dirroot . '/mod/customdocument/locallib.php');

        if ($certificateinstanceid = $DB->get_record("customdocument", array('name' => $certificatename), 'id')) {
            $certificateinstanceid = $certificateinstanceid->id;
        } else {
            throw new Exception("Can't find certificate with name: $certificatename");
        }

        $cm = get_coursemodule_from_instance('customdocument', $certificateinstanceid, 0, false, MUST_EXIST);
        ...
        if (empty($issuecert->has_change)) {
            $customdocument->get_issue_file($issuecert);
        }
        return $issuecert;
    ...
    public function issue_certificate_to($certificatename, $username) {
        ...
        if (!$issuecert = $this->get_issue_certificate_by_customdocument_name($certificatename, $user)) {
            throw new Exception("Can't find certificate with name: $certificatename");
        }
