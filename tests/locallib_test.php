 ...
 * @copyright 2013 onwards Carlos Alexandre S. da Fonseca
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @group customdocument_basics
 */

// ...vendor/bin/phpunit mod_customdocument_locallib_testcase mod/customdocument/tests/locallib_test.php .
class mod_customdocument_locallib_testcase extends mod_customdocument_base_testcase {

    public function test_create_instance() {
        global $DB;

        // Basic CRUD test.
        $this->assertFalse($DB->record_exists('customdocument', array('course' => $this->course->id)));
        $cert = $this->create_instance();
        $this->assertEquals(1, $DB->count_records('customdocument', array('course' => $this->course->id)));
        $this->assertTrue($DB->record_exists('customdocument',
        ...
        $this->assertEquals(2, $DB->count_records('customdocument', array('course' => $this->course->id)));
        $this->assertEquals('One more certificate',
                            $DB->get_field_select('customdocument', 'name', 'id = :id',
    ...
    public function test_update_instance() {
        ...
        $instancedb = $DB->get_record('customdocument', array('id' => $cert->get_instance()->id));
    ...
    public function test_certificate_images() {
        ...
        '/mod/customdocument/tests/test_certificate_'.testable_customdocument::PLUGIN_VERSION.'.pdf';
    ...
    public function test_create_issue_instance() {
        ...
        $this->assertFalse($DB->record_exists("customdocument_issues",
        ...
        $this->assertTrue($DB->record_exists("customdocument_issues", array('id' => $issuecert->id)));
        ...
        $this->assertNotEmpty($issuecert);
        $this->assertFalse($DB->record_exists("customdocument_issues", array('id' => $issuecert->id)));
        $this->assertTrue(!empty($issuecert->haschange));
        ...
        $this->assertTrue($DB->record_exists("customdocument_issues", array('id' => $issuecert->id)));
        $this->assertEquals(1,
                            $DB->count_records("customdocument_issues",
                                            array('certificateid' => $cert->get_instance()->id,
                                                  'userid' => $this->students[1]->id))
        );
        $this->assertTrue(!empty($issuecert->haschange));
        ...
        $this->assertEquals(2, $DB->count_records("customdocument_issues",
                        array('certificateid' => $cert->get_instance()->id)));
    }
    ...
    public function test_create_issue_code() {
        ...
        $this->assertEquals($this->students[0]->id,
                            $DB->get_field_select('customdocument_issues', 'userid', 'code = :code',
    ...
    public function test_update_instace_update_haschange_issues() {
        ...
        $this->assertTrue($DB->update_record('customdocument_issues', $issuecert1));
        $issuecert2->haschange = 0;
        $this->assertTrue($DB->update_record('customdocument_issues', $issuecert2));
        $issuecert3->haschange = 0;
        $this->assertTrue($DB->update_record('customdocument_issues', $issuecert3));
        ...
        // Update customdocument instance.
    ...
    public function test_detete_instace_update_timedelete_issues() {
        ...
        $issuecert1 = $DB->get_record('customdocument_issues', array('id' => $issuecert1->id));
...
//     }
    ...
    public function test_cant_issue_with_grade_restrinction() {
        ...
        $this->assertEquals(get_string('cantissue', 'customdocument'), $mock_cert->testable_can_issue($this->students[0]));
    }
    ...
    public function test_cant_issue_with_completion_restrinction() {
        ...
        $error_msg = get_string('requiredtimenotmet', 'customdocument', $a);
    ...
    private function get_certificate_mock(array $methods = null, array $params = array(), array $options = null) {
        ...
        $mockcert = $this->getMockBuilder('testable_customdocument')->setMethods($methods)->setConstructorArgs(array($cert->get_context(),
