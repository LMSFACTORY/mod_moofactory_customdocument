...
function send_certificate_file(stdClass $issuedcert) {
    ...
    if ($issuedcert->haschange) {
        ...
        if (empty($issuedcert->timedeleted)) {
            ...
            try {
                // Try to get cm.
                $cm = get_coursemodule_from_instance('customdocument', $issuedcert->certificateid, 0, false, MUST_EXIST);
                ...
                $file = $customdocument->get_issue_file($issuedcert);
...
function put_watermark($file) {
    ...
    for ($pgnum = 1; $pgnum <= $pagecount; $pgnum++) {
        ...
        $pdf->Cell($size['width'], $fontsize, get_string('certificatecopy', 'customdocument'), $bodersytle, 0, 'C', false, '',
                4, true, 'C', 'C');
        $pdf->StopTransform();
